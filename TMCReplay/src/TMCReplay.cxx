...
Bool_t TMCReplay::GetShape(const TString& volumePath, TString& shapeType, TArrayD& par)
{
  Int_t npar;
  fGeoManager->PushPath();
  if (!fGeoManager->cd(volumePath.Data())) {
...
Bool_t TMCReplay::GetMaterial(const TString& volumeName, TString& name, Int_t& imat, Double_t& a, Double_t& z, Double_t& density,
...
Bool_t TMCReplay::GetMedium(const TString& volumeName, TString& name, Int_t& imed, Int_t& nmat, Int_t& isvol, Int_t& ifield,
...
Int_t TMCReplay::VolId(const char* volName) const
{
...
const char* TMCReplay::VolName(Int_t id) const
{
...
Int_t TMCReplay::MediumId(const char* mediumName) const
{
...
Int_t TMCReplay::NofVolumes() const
{
  return fGeoManager->GetListOfVolumes()->GetEntriesFast() - 1;
}

Int_t TMCReplay::VolId2Mate(Int_t id) const
{
  auto volume = fGeoManager->GetVolume(id);
  if (!volume) {
...
Int_t TMCReplay::NofVolDaughters(const char* volName) const
{
...
const char* TMCReplay::VolDaughterName(const char* volName, Int_t i) const
{
  // Get volume
  auto volume = fGeoManager->GetVolume(volName);
  if (!volume) {
...
Int_t TMCReplay::VolDaughterCopyNo(const char* volName, Int_t i) const
{
  // Get volume
  auto volume = fGeoManager->GetVolume(volName);
  if (!volume) {
...
Bool_t TMCReplay::SetCut(const char* cutName, Double_t cutValue)
{
...
bool TMCReplay::keepDueToProcesses(const o2::StepInfo& step) const
{
  return true;
}

bool TMCReplay::keepDueToCuts(const o2::StepInfo& step) const
{
...
bool TMCReplay::keepStep(const o2::StepInfo& step) const {
...
void TMCReplay::ProcessEvent(Int_t eventId)
{
...
Bool_t TMCReplay::ProcessRun(Int_t nevent)
{
  fCurrentEvent = 0;
  ...
  if(!treeReader.changeToTTree(fTreename)) {
    TMCReplay::Fatal("Cannot find TTree %s", fTreename.c_str());
  }
  ...
  if(!treeReader.setBranch("Steps", &fCurrentStepInfo) || !treeReader.setBranch("Calls", &fCurrentMagCallInfo) || !treeReader.setBranch("Lookups", &fCurrentLookups)) {
    treeReader.close();
    TMCReplay::Fatal("Cannot find required branches in TTree %s", fTreename.c_str());
  }
